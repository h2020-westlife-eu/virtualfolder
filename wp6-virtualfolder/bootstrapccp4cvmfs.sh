#!/usr/bin/env bash
# download ccp4, configure and install
set LOCAL=/etc/cvmfs/default.local

echo CVMFS_REPOSITORIES=facilities.gridpp.ac.uk >/etc/cvmfs/default.local
echo CVMFS_DEBUGLOG=/tmp/cvmfs.log >>/etc/cvmfs/default.local
echo CVMFS_HTTP_PROXY=\"http://cvmfs-squid.gridpp.rl.ac.uk:3128\" >>/etc/cvmfs/default.local

set GRIDPP=/etc/cvmfs/domain.d/gridpp.ac.uk.conf
#exec 3> $GRIDPP
echo "CVMFS_SERVER_URL=\"http://cvmfs-egi.gridpp.rl.ac.uk:8000/cvmfs/\@org\@.gridpp.ac.uk;http://cvmfs01.nikhef.nl/cvmfs/\@org\@.gridpp.ac.uk\"" >/etc/cvmfs/domain.d/gridpp.ac.uk.conf
echo CVMFS_PUBLIC_KEY=/etc/cvmfs/keys/gridpp.ac.uk.pub >>/etc/cvmfs/domain.d/gridpp.ac.uk.conf

set GRIDPPKEY=/etc/cvmfs/keys/gridpp.ac.uk.pub

echo "-----BEGIN PUBLIC KEY-----

DwIDAQAB
-----END PUBLIC KEY-----
">/etc/cvmfs/keys/gridpp.ac.uk.pub
service autofs restart
cvmfs_config probe
